...
import IncomeEdit from "../components/modal/IncomeEdit";
  ...
  const deleteIncome = async (id: number | undefined) => {
    try {
      const removeIncome = await fetch(
        `${process.env.REACT_APP_SERVERURL}/api/v1/income/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleDateChange = async (dates: any) => {
    try {
      ...
      const getData = await fetch(
        `${process.env.REACT_APP_SERVERURL}/api/v1/income/date/${queryString}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          ...
          <Card>
            <Card.Header>
              <h4 style={{ textAlign: "center", textTransform: "uppercase" }}>
                Total Income <i className="bi bi-currency-rupee"></i>
                {incomeAmount}{" "}
              </h4>
            </Card.Header>
          </Card>
